...
const DisplayedUserCard: React.FC<DisplayedUserCardProps> = ({
  ...
  const handleAccept = async () => {
    ...
    } // open modal if card is going from interested to matched
    fetchFunction(); // async await to ensure axios request completes before fetching data
  };

  const handleReject = async () => {
    if (userIsTutor) {
      // make student unlike tutor
      console.log("Student rejected");
      await axios
        .delete(
          `http://localhost:8080/student/unlike/${displayedUser.id}/${userID}`
        )
        .catch((error) => {
      ...
      await axios
        .delete(
          `http://localhost:8080/tutor/unlike/${displayedUser.id}/${userID}`
        )
        .catch((error) => {
  ...
  return (
    <div className="flex flex-col p-4 w-72 bg-lightbiege rounded-lg shadow-lg min-h-[208px]">
      ...
      {!userIsTutor && (
        <p className="font-light">${displayedUser.pricing} /hr</p>
      )}

      {cardType == "match" && (
        <div className="flex justify-between mt-auto px-2">
          <button
            className="px-4 w-20 h-10 rounded-md shadow-md text-gray-700 bg-violet-200 hover:bg-violet-300"
            onClick={handleChat}
          >
            Chat
          </button>
          <button
            className="px-4 min-w-fit h-10 text-center rounded-md shadow-md text-gray-700 bg-gray-200 hover:bg-gray-300"
      ...
      {cardType == "interested" && (
        <div className="flex justify-between mt-auto px-2">
          <button
            className="px-4 w-20 h-10 rounded-md shadow-md text-gray-700 bg-lightbrown hover:bg-amber-50"
            onClick={handleAccept}
          >
            Match
          </button>
          <button
            className="px-4 w-20 h-10 rounded-md shadow-md bg-mattebrown hover:bg-gray-300 text-dirtwhite"
      ...
      {cardType == "other" && (
        <div className="flex justify-center mt-auto">
          <button
            className="px-4 flex items-center justify-center w-20 h-10 rounded-md shadow-md text-gray-700 bg-lightbrown hover:bg-amber-50"
            onClick={handleAccept}
          >
            Yes
          </button>
